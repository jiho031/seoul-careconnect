...
public final class PolicySpecification {
    ...
    public static Specification<Policy> build(
            PolicySearchDTO search,
            LocalDate today,
            int newDays
    ) {
        return (root, query, cb) -> {
            ...
            if (keyword != null) {
                ...
                predicates.add(cb.or(
                        ...
                        cb.like(
                                cb.lower(root.get("applyMethod")),
                                pattern
                        ),
                        cb.like(
                                cb.lower(source.get("sourceName")),
                                pattern
                        )
                ));
            }
            ...
            parseEnum(
                    ...
                    .ifPresent(value -> {
                        String agePattern =
                                likePattern(value.getLabel());

                        String allPattern =
                                likePattern(AgeGroup.ALL.getLabel());
            ...
            String targetKeyword =
                    normalize(search.getTargetKeyword());

            if (targetKeyword != null) {
                String pattern =
                        likePattern(targetKeyword);
            ...
            parseEnum(
                    ApplyStatus.class,
                    search.getApplyStatus()
            )
                    .filter(value ->
                            value != ApplyStatus.EXPIRED
                    )
                    .ifPresent(value ->
                            predicates.add(
                                    cb.equal(
            ...
            switch (search.safeQuick()) {
                case "open" -> predicates.add(
                        root.get("applyStatus").in(
                                ApplyStatus.OPEN,
            ...
            if ("deadline".equals(search.safeSort())
                    && !isCountQuery(query.getResultType())) {
